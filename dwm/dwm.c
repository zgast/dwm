...
createmon(void)
{
	...
	for (i=0; i <= LENGTH(tags); i++) {
		...
		/* init layouts */
		m->pertag->ltidxs[i][0] = m->lt[0];		m->pertag->ltidxs[i][1] = m->lt[1];
...
setlayout(const Arg *arg)
{
	...
	selmon->lt[selmon->sellt] = selmon->pertag->ltidxs[selmon->pertag->curtag][selmon->sellt];	strncpy(selmon->ltsymbol, selmon->lt[selmon->sellt]->symbol, sizeof selmon->ltsymbol);
	if (selmon->sel)
		arrange(selmon);
	else
		drawbar(selmon);
